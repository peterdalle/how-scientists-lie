# QRPs through history {- #qrps-history}
...
- [the usage of the term](#qrps-term): the etymology of the term questionalbe research practices.
...
> 
> <mark>TRIMMING consists in clipping off little bits here and there from those observations which differ most in excess from the mean</mark>, and in sticking them on to those which are too small; a species of "equitable adjustment," as a radical would term it, which cannot be admitted in science.
> 
...
```{r qrp-term-usage,fig.width = 6, fig.height = 4, fig.cap="Use of the term *questionable research practices* over time. Data from the database Scopus up until 2021."}
library(yaml)
library(dplyr)
library(ggplot2)
...
data |>
  ...
  ggplot(aes(year, hits)) + 
    ...
    scale_y_continuous(breaks = seq.int(0, 60, by=10),
                      limits = c(0, 60),
                      labels = c(0, 10, 20, 30, 40, 50, "60 docs")) +
    theme_howscientistslie() +
    labs(title = '"Questionable research practices" over time', 
        subtitle = "Number of documents that mention the phrase",
        x = NULL,
        y = NULL) +

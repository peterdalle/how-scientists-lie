...
There are several remedies and countermeasures to prevent questionable research practices (QRPs). The remedies presented here are meant to be exhaustive and provide a brief overview of the advantages and disadvantages of each remedy. In this way, you will be able to find a remedy that is suitable for you and your circumstances as well as suggest an appropriate remedy for other researchers.
...
Power analysis is a method to estimate the required sample size for a statistical test in order to be able to find a particular size difference or effect size.

Note that so called *post hoc power* is a meaningless concept and should be discouraged.^[The post hoc power can be estimated from the p-value since they are formally equivalent. For example, if studies are selectively reported based on p-values then the post hoc power will reflect selective reporting and thus not inform us whether the study had enough power.]

Power analysis is only appropriate for frequentist statistics, not for [Bayesian statistics](#bayes).

::: {.rmdtip}
Free software that helps with power analysis: G*Power
:::
    
### Replication {#replication}

::: {.rmdnote}
The terms "reproducibility" and "replication" are sometimes used interchangeably. Here, reproducibility refers to computational reproducibility, which is when an analaysis or procedure can be performed again with identical or similar result with a computer script or equivalent.
:::

### Change significance threshold {#change-alpha}

In the social sciences, it is common to use p < .05 as a threshold for statistical significance---which is used as a threshold for a discovery or positive finding. This means that we allow 5 percent false positives by chance alone (on average). In combination with publication bias and questionable research practices, the false positive rate is actually much higher.
...
Preregistration means registering hypotheses, methods or analyses of a study before it is conducted. A preregistration is a document that details what the hyptheosis is, what methods are used, how data collection and analysis is carried out, and so on. The preregistration can be submitted to one of many online repositories where it recieves a timestamp. You can then choose if you want to make it public immediately or wait until publication.

Preregistration does not prevent questionable research practices from occuring, but makes it transparent what the authors said they were going to do and what they eventually did. For instance, if a researcher preregister an experimental hypothesis and later changes the hypothesis to fit the evidence (so called [HARKing](#harking)), readers will be able to tell from reading both the preregistration and the article. Likewise, if data collection or data analysis were not mentioned in the preregistration, then it would also be apparent that the choices related to this (such as [outlier exclusion](#outlier-exclusion)) were made after the fact.

There are many parts of the research process that can be preregistered. Primary data collection, such as an experiment, can be preregistered. But also secondary data analyses such as systematic reviews and meta-analyses.
...
However, the efficity of retractions can be questioned. Some studies have shown that retracted articles are still cited many years afterwards.
...
As @gelman_2016 have argued, the standard view seems to be that "once an article is published in some approved venue, it should be taken as truth". However, he also noted that this is inadequate for science since "many published papers are clearly in error" and that "publication itself is such a crapshoot that it's a statistical error to draw a bright line between published and unpublished work".

Post-publication peer review emphasize peer review as ongoing and does not stop just because something is published. However, there are no formal ways of conducting peer review after publication and many journals are reluctant to publishing criticism of already published articles.

For this reason, alteratives such as [PubPeer](https://pubpeer.com/) has entered the scence, whih is a website that provides a way of finding comments about any article published in any journal, like a layer on top of all journals. You can also post comments yourself. This is an easy way of doing post-publication peer review, and through a browser plugin PubPeer can also notify you when you visit a journal article that has comments on PubPeer.

### Blogs and social media

Using social media or blogs to call attention to questionable research practices is an increasingly important way that research is carried out, which is an informal part of the [post-publication peer review](#post-publication-peer-review). Scientific journals are primarily occupied with formal article formats (such as empirical articles, reviews, or commentaries) and it can be difficult and time-consuming to critisize research in a way that is suitable for one of those predefined article formats.
...
Blogs and social media are not without problems of their own. They do not become part of the scientific record and it can be difficult to backtrace their references that might break. However, this is similar to chats at academic conferences where a handful of people talk to each other about bad research and researchers without any written account that others can see. Blogs and social media at least makes it possible for informal conference talk to surface in a more transparent way than before.

::: {.rmdtip}
Use services such as <https://web.archive.org/> to find (and link to) old blog posts that are not available online anymore.
:::
...
An adversarial collaboration is when two or more scientists with opposing views work together. Sometimes it is also called *red team/blue team*.
...
These remedies presented here are are no cure-all for questionable research practices, nor will they stop all unethical behaviors. A researcher who wants to lie will find a way. However, the remedies nonetheless makes research a little more difficult to fake (and conversely, easier for others to identify), which is an important first step.
